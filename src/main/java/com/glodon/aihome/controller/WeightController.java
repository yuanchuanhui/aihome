...
public class WeightController {
    ...
    public String getWeight(String mealType){
        return getWeight(mealType, Calendar.getInstance().get(Calendar.DAY_OF_YEAR), user);
    }

    public static String getWeight(String mealType, int key, User user){
        // 随机生成重量
        float weight = (float) (Math.random() * 0.5);
        // 根据日期取出今天的营养对象
        NutritionOfDay nutritionOfDay = user.getMeals()
                .getOrDefault(key, new NutritionOfDay());
        // 根据食物种类得到其营养组成
        ...
        // 更新所有日期的map
        user.getMeals().put(key, nutritionOfDay);
